...
The stale-cosmetic banner repair from the v5.28 run is retained (it is a
no-op now that the banner was already fixed) so the rule set stays uniform.

Note: the untracked OTTO-v5.00/ snapshot is a DIFFERENT tree pinned at
v5.27 and is deliberately NOT in FILES; only the live files at the repo
root are bumped.
...
# v5.26/v5.27 engine notes in COttoCorrelationFilter.mqh are never touched.
PATTERNS += [
