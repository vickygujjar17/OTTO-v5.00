...
Also repairs two stale cosmetic banners that the v5.27 bump could not match:
  * otto.mq5 line 4 still read "Pine Script Master Build Port (v5.26)"
    because the v5.27 pattern required "(v<n>)" to follow "Master Build "
    directly, and here it follows "Master Build Port ".
  * OttoDefines.mqh line 3 ends with " |" rather than ")", so it is covered
    by the OTTO-EA-v pattern instead.
...
# v5.27 engine notes in COttoCorrelationFilter.mqh are never touched.
PATTERNS += [
...
# Cosmetic banner that has been stale since the (v5.26) release: the port
# banner in the otto.mq5 header. Kept literal rather than OLD-based because the
# text it carries is two releases behind the file it sits in.
STALE_PORT_BANNER = (
    re.compile(r'(Pine Script Master Build Port \(v)5\.26(\))'),
    r'\g<1>' + NEW + r'\g<2>',
)
...
def bump(path):
    ...
    for i, line in enumerate(lines):
        ...
        # The stale port banner is never inside a FIX annotation, but the
        # guard above is shared so the rule stays uniform.
        new_line = STALE_PORT_BANNER[0].sub(STALE_PORT_BANNER[1], new_line)
        if new_line != line:
            print("  %s:%d" % (path, i + 1))
            print("    - %s" % line.strip())
